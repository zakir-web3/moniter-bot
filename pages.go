...
func publishToPages(ctx context.Context, token, pagesRepo, repo string, r *Release, analysis string) error {
	now := time.Now()
	slug := sanitizeSlug(repo, r.TagName)
	path := fmt.Sprintf("_posts/%s-%s.md", now.Format("2006-01-02"), slug)

	md := buildPostMarkdown(repo, r, analysis, now)

	reqBody := createFileRequest{
		Message: fmt.Sprintf("docs: add analysis for %s %s", repo, r.TagName),
...
func buildPostMarkdown(repo string, r *Release, analysis string, now time.Time) string {
	...
	fmt.Fprintf(&b, "> 分析日期: %s | [查看原始发布说明](%s)\n\n", now.Format("2006-01-02"), r.HTMLURL)
	b.WriteString("---\n\n")
